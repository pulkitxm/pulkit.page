...
function slider({ id, label, key, value, min, max, step }) {
  return html`<div class="flex flex-col gap-1.5">
    <div class="flex items-center justify-between">
      <label for="${id}" class="${sliderLabel}">${label}</label>
      <span data-ref="${key}Value" class="${sliderValue}">${value}</span>
    </div>
...
export function mount(root) {
  ...
  root.innerHTML = html`<div class="flex h-full w-full flex-col items-center justify-center gap-6 p-6">
    <div class="w-full max-w-sm space-y-4">
      ${slider({ id: `${id}-stiffness`, label: "Stiffness", key: "stiffness", value: 300, min: 50, max: 1000, step: 10 })}
      ${slider({ id: `${id}-damping`, label: "Damping", key: "damping", value: 20, min: 1, max: 100, step: 1 })}
      ${slider({ id: `${id}-mass`, label: "Mass", key: "mass", value: 1, min: 0.1, max: 5, step: 0.1 })}
    </div>
