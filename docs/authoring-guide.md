...
Styling uses Tailwind CSS v4. [styles.css](../styles.css) is the Tailwind entry: it replaces the default theme with the site's tokens (warm light/dark colors, syntax colors, a few font sizes, a 560-pixel `sm` breakpoint), skips Preflight so Markdown elements keep browser defaults, and styles Markdown output (headings, paragraphs, code, tables, images, quotes) in the base layer because generated prose carries no classes. Layouts and [render-page.mjs](../scripts/render-page.mjs) use utility classes for the centered 760-pixel shell, fluid H1 sizing, header, footer, listings, breadcrumbs, and the skip link. Below 560 pixels (`max-sm:`) the header stacks and padding tightens. Tailwind scans only `layouts/`, `render-page.mjs`, and `highlight.mjs`, so classes must appear as complete literal strings there. Browser CSS has no `@font-face` rules. Social-card generation separately uses the bundled IBM Plex Mono TTF with system fonts disabled. There is no component library or animation runtime in the active site.
