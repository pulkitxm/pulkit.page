...
[home.html](../layouts/home.html), [simple.html](../layouts/simple.html), and [article.html](../layouts/article.html) share the same semantic structure: English HTML document, skip link to `main`, site header/nav, main containing an article, one page H1, prose container, and footer. Their main class differs. Home uses smaller H2 headings via CSS; article/simple currently share most styling. Changing a template never creates a route.
...
[styles.css](../styles.css) uses warm light/dark CSS variables, system sans-serif body fonts, system monospace code fonts, a centered 760-pixel shell, fluid H1 sizing, focus outlines, a keyboard skip link, responsive images, and horizontally scrolling code/tables. Below 560 pixels the header stacks and padding tightens. Browser CSS has no `@font-face` rules. Social-card generation separately uses the bundled IBM Plex Mono TTF with system fonts disabled. There is no Tailwind, CSS compilation, component library, or animation runtime in the active site.
