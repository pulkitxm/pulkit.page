...
| [work-with-me.md](../apps/page/content/work-with-me.md) | `/work-with-me/` | A figures band, a carousel of past sites, services and process as card grids, the Claude Directory/Hinge/Warden posts with their replies carousel, a scope and timeline table, grouped tech badges, an FAQ in `details` blocks, and booking links |
...
[apps/page/assets/](../apps/page/assets/) holds 61 tracked files: experience screenshots, photos, and documents under `assets/content/exp/`, company logos under `assets/exp/`, and the separately supplied résumé. [apps/blog/assets/content/](../apps/blog/assets/content/) holds 198 tracked files: post images under historical topic directory names in `blogs/`, public-gallery images under `public/blogs/`, and a series video under `public/series/`. Image folder names need not match current post slugs: the Markdown URL is authoritative. The historical audit lists 197 copied images. Fonts, favicons, and the author portrait live in `packages/theme/assets/`. Build copies those shared assets and then the app's whole asset tree on top, regardless of whether a page references each file.

A full build writes 39 generated files for pulkit.page and 166 for pulkit.blog, one HTML page, one social card and one Markdown copy per route plus the crawler files; see [SEO and environments](seo-and-environments.md#cards-and-crawler-files).
