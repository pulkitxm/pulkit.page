...
This is a static publishing system for a portfolio and a substantial writing archive, organized as a Bun and Turborepo monorepo. The site lives in `apps/page`, and reusable pieces live in workspace packages: the generator in `packages/engine`, code formatting and highlighting in `packages/code`, Markdown embeds in `packages/embeds`, interactive demos in `packages/demos`, and the shared theme in `packages/theme`. Repository-wide gates live in `tooling/checks`. Markdown defines the pages. Nothing generated is committed; a build renders HTML, social cards, sitemap, and robots output into `apps/page/dist/` for the production or a preview origin. Visitors need no application server or JavaScript framework; site JavaScript is limited to the theme toggle, carousels, embeds, and demos.
...
4. [Content inventory](content-inventory.md): all page types, portfolio pages, experience, metadata, and the linked article catalog.
5. [Article catalog](article-catalog.md): every article, its date, subject, and tags.
...
Canonical URLs, Open Graph and Twitter metadata, JSON-LD, social-card PNGs, sitemap, breadcrumbs, and related writing are implemented. Production uses the app's CNAME; development and custom environments use their resolved origin. See [SEO and environments](seo-and-environments.md) for the complete behavior and limitations.
