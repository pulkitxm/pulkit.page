...
pulkit.page has 14 content Markdown files, 13 authored pages plus the shared not-found page, and 61 tracked app assets, and its build renders 46 files (15 HTML, counting the `404.html` copy, 14 PNG cards, 14 Markdown copies, `llms.txt`, sitemap, robots) into `apps/page/dist/`. pulkit.blog has 56 content Markdown files, 55 authored pages plus the same not-found page, and 198 tracked app assets, and its build renders 173 files (57 HTML, counting the `404.html` copy, 56 PNG cards, 56 Markdown copies, `llms.txt`, sitemap, robots, feed) into `apps/blog/dist/`. None of the generated files are committed. The historical migration audit still records 58 imported sources and 197 copied assets.
