...
For example, `blogs/system-design/caching.mdx` maps to [content/blogs/system-design/caching.md](../apps/page/content/blogs/system-design/caching.md), with 30 original code blocks. `experiences/magicapi.mdx` maps to [content/exp/magicapi.md](../apps/page/content/exp/magicapi.md): the plural directory becomes the shorter `exp` route. The source field is not relative to `docs/` or the repository root, and the output field predates the workspace split: its `content/` and `assets/` paths now live under `apps/page/`.
...
The audit omits the separately supplied résumé PDF, the fonts now kept in `packages/theme/fonts/`, and media added when the experience write-ups were restored. It is therefore unsuitable as an exhaustive current asset manifest. Build copies the entire `apps/page/assets/` tree without consulting it. The 66 generated social cards are rendered into `apps/page/dist/og/` by the build, outside this copied-source inventory.

## Who reads it

A one-time importer, since removed from the repository, wrote this JSON while converting the reference MDX into Markdown. No current code reads the audit: the engine discovers files in `apps/page/content/` and renders the selected environment into `apps/page/dist/`, and CI never consults its fields. Generic JSON formatting, repository path/size checks, and text-policy checks inspect it as ordinary repository data, but never use its contents to select routes or verify migration completeness.
...
The static project preserves the article archive, the full experience write-ups, and selected imagery. Rich widgets were first flattened into readable static equivalents or links; later work restored many of them as build-time embeds in `packages/embeds` and interactive demos in `packages/demos`. Home, About, Contact, Writing landing, Experience landing, Résumé, tools, and services are authored separately.
