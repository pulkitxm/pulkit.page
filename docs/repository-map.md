...
| Package                                  | Contents                                                                                                                                                                                                                                                                         |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [packages/engine](../packages/engine/)   | `src/cli.ts` (the `site` bin) and `src/commands/`, inventory, rendering, layouts, generation, build, Vite development server, preview, SEO, social cards, Markdown copies and `llms.txt`, render cache, origin resolution, post-build checks, and their tests                    |
...
| [packages/embeds](../packages/embeds/)   | `src/renderers/*.ts` renderers for `:::embed` and `:embed[...]` with `src/renderers/registry.ts`, `src/index.ts`, raw HTML rules in `src/raw-html/`, helpers in `src/lib/`, browser scripts in `client/`, and `src/bundle.ts` for bundling them with KaTeX and PhotoSwipe assets |
| [packages/demos](../packages/demos/)     | Browser code in `client/` (`index.ts`, `registry.ts`, `types.ts`, `runtime/`, `lib/`, `components/`), `showcases/*.json`, `styles.css`, plus `src/render.ts` for `:::demo` and `src/assets.ts` for CSS, Poppins fonts, and script bundles                                        |
...
Start with the [architecture overview](architecture.md), then read [site-inventory.ts](../packages/engine/src/site/site-inventory.ts) for route discovery and [generate.ts](../packages/engine/src/site/generate.ts) for writing the rendered site, then [render-page.ts](../packages/engine/src/render/render-page.ts) for Markdown and metadata behavior. Next, [layouts.ts](../packages/engine/src/render/layouts.ts) explains template substitution, and [build.ts](../packages/engine/src/commands/build.ts) shows rendering into `dist/` followed by asset copying. [cli.ts](../packages/engine/src/cli.ts) maps each `site` subcommand to its module. The [rendering flow](rendering-flow.md) connects these functions using real examples.
