...
| `bun run lighthouse`       | Build, then [lighthouse.mjs](../scripts/lighthouse.mjs) with installed Chrome     | Audits every built route on mobile and desktop; writes Markdown summary, per-page reports and HTML to ignored `reports/lighthouse`. Options: route arguments, `--form-factor`, `--origin`, `--concurrency` |
