...
| -------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `bun run dev`              | `turbo run dev`                                                              | Starts every app's development server; persistent and never cached                                         |
...
`check-content.mjs` accepts only an optional `--write`. To run one Turbo task for a single app, filter it, for example `bunx turbo run dev --filter=@pulkit/page`.

## App commands

[apps/page/package.json](../apps/page/package.json) defines the site's scripts. Each one except `check:html` calls the `site` bin from `@pulkit/engine`. [cli.mjs](../packages/engine/src/cli.mjs) maps the subcommand to a module in `packages/engine/src/` and runs it with Bun in the app directory. Run a script directly with `bun run --cwd apps/page <script>`; direct runs skip Turbo's dependency ordering, so post-build checks need an existing `dist/`.
...
| `build`         | `site build`: [build.mjs](../packages/engine/src/build.mjs) calls `generateSite` from [generate.mjs](../packages/engine/src/generate.mjs)   | Clears `dist/` except dev roots, renders HTML, cards, sitemap and robots with the render cache, then copies and compiles assets |
...
| [site-inventory.mjs](../packages/engine/src/site-inventory.mjs)     | Generator, dev renderer, checks            | Walks `content/`, validates routes, loads shared metadata                                                                  |
...
| [seo.mjs](../packages/engine/src/seo.mjs)                           | Renderer, cards, checker, tests            | Full title, card paths, ancestors, related scores, graph construction, safe JSON                                           |
| [og-images.mjs](../packages/engine/src/og-images.mjs)               | Generator and tests                        | resvg PNG rasterization with the bundled TTF, sitemap/robots buffers                                                       |
...
The development server runs in the app directory and binds Vite to `127.0.0.1`. Unset PORT prefers 3000 and tries higher ports when occupied. Explicit occupied ports fail; PORT=0 requests an available port. Canonical URLs use the actual bound origin. Vite's file access is limited to the app directory and the repository root, so workspace packages can be served.
...
- Verify runs `bun run ci`, uploads `apps/*/dist` as the `built-sites` artifact, and on main push/manual dispatch uploads `apps/page/dist` as the Pages artifact.
