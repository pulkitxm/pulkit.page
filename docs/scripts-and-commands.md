...
`check-content.mjs` accepts only an optional `--write`. To run one Turbo task for a single app, filter it, for example `bunx turbo run dev --filter=@pulkit/page` or `bunx turbo run dev --filter=@pulkit/blog`.
...
- Verify runs `bun run ci` (with Vercel's remote Turbo cache when `TURBO_TOKEN` and `TURBO_TEAM` are set) and uploads `apps/*/dist` (both sites) as the `built-sites` artifact.
...
The `CI` gate depends on all jobs and fails for failure, cancellation, or skipped results. The deploy job depends on that gate and runs once per app. It downloads `built-sites` and uploads that app's `dist/` to its Vercel project with the Vercel CLI: production for main pushes and manual dispatch, preview for pull requests from this repository. It records the URL on a GitHub environment (`page`, `blog`, `page-preview`, `blog-preview`). See [deployment](deployment.md) for the secrets and variables it reads. Actions are pinned to commit hashes. Concurrency groups by ref and cancels in-progress pull-request runs. These YAML intentions do not establish remote branch protection, environment approvals, or DNS state.
