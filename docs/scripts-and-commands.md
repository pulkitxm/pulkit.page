...
| `bun run build`            | [build.mjs](../scripts/build.mjs) calls `generateSite` from [generate.mjs](../scripts/generate.mjs) | Clears dist except dev roots, renders HTML, cards, sitemap and robots with the render cache, then copies and compiles assets |
...
| `bun run check:shell`      | `sh -n` on install-hooks.sh and pre-commit                                                          | Syntax only; does not execute their behavior                                                                                 |
...
| `bun run lint`             | `biome check --error-on-warnings .`, then content checker                                           | Read-only; warnings fail; later stages run only if earlier stages succeed                                                    |
| `bun run check:html`       | html-validate on `dist/**/*.html`                                                                   | Read-only; requires a prior build; CI runs it after build                                                                    |
...
`generate.mjs` is a module, not a CLI; the build imports its `generateSite`. `check-content.mjs` accepts only an optional `--write`. Most other scripts are narrow internal entry points rather than general-purpose CLIs.
...
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| [site-origin.mjs](../scripts/site-origin.mjs)           | Generator/build/SEO checker/tests; validates CNAME or environment origin                                       |
...
| [site-origin.test.js](../scripts/site-origin.test.js)   | Origin precedence, invalid URLs, and CNAME changes                                                             |
...
It neither stages repairs nor changes the original index/worktree. If a correction exists only unstaged, the staged version still fails; the regression test explicitly covers this. Untracked documentation is checked by local CI but will not be in a commit snapshot until staged. The hook uses current installed dependencies, not a fresh install from the staged lockfile, so remote frozen installation is an additional safeguard. Installation can fail rather than overwrite a different `core.hooksPath`; integrate deliberately if using another hook manager.
...
`bun run build` and Vite development share the checksummed render cache in `.cache/generate/`. It can be deleted to force a cold rebuild; it is never needed for correctness. Builds reuse HTML, fences, and cards and report counts of newly computed cached items; an empty count means all results were reused.
