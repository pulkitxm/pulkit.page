...
Sitemap includes every discovered page exactly once with its canonical URL, except the not-found page, which is `noindex, follow` and stays out of the sitemap, the feed and `llms.txt`. Articles carry `lastmod` from their publication `date`; other pages have no trustworthy date and stay undated rather than getting an invented one. Robots allows crawling and advertises that sitemap.
...
pulkit.page's output comprises 15 HTML files (14 pages, with the not-found page copied to `404.html`), 14 cards, 14 Markdown copies, `llms.txt`, sitemap, and robots; pulkit.blog's comprises 57 HTML files (56 pages, copied the same way), 56 cards, 56 Markdown copies, `llms.txt`, sitemap, robots, and `feed.xml`. Cards are generation outputs, not part of the historical copied-image audit.
