import { createRequire } from "node:module";
import { join } from "node:path";
import type { Biome, Configuration } from "@biomejs/js-api/nodejs";
import { readJson } from "@pulkit/shared/files";
import { repositoryRoot } from "@pulkit/shared/repository";

interface Workspace {
  biome: Biome;
  projectKey: number;
}

const require = createRequire(import.meta.url);
let workspace: Workspace | undefined;
const literal = new Set(["", "text", "plaintext", "txt", "math", "mermaid"]);
const biomeFiles = new Map([
  ["js", "snippet.js"],
  ["javascript", "snippet.js"],
  ["mjs", "snippet.mjs"],
  ["cjs", "snippet.cjs"],
  ["jsx", "snippet.jsx"],
  ["ts", "snippet.ts"],
  ["typescript", "snippet.ts"],
  ["mts", "snippet.mts"],
  ["cts", "snippet.cts"],
  ["tsx", "snippet.tsx"],
  ["json", "snippet.json"],
  ["jsonc", "snippet.jsonc"],
  ["css", "snippet.css"],
  ["html", "snippet.html"],
  ["graphql", "snippet.graphql"],
  ["gql", "snippet.graphql"],
]);

function isConfiguration(value: unknown): value is Configuration {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function repositoryConfiguration(): Configuration {
  const configuration = readJson(join(repositoryRoot, "biome.json"));
  if (!isConfiguration(configuration)) {
    throw new Error("biome.json must contain a configuration object");
  }
  return { ...configuration, vcs: { enabled: false } };
}

const configuration = repositoryConfiguration();

function linesOf(code: string): string[] {
  return code
    .replace(/\r\n|\r/g, "\n")
    .split("\n")
    .map((line) => line.replace(/[ \t]+$/, ""));
}

function trimTrailingBlanks(lines: readonly string[]): string[] {
  const next = [...lines];
  while (next.length > 0 && next.at(-1) === "") {
    next.pop();
  }
  return next;
}

function expandLeadingTabs(line: string): string {
  const indent = /^(?:\t| )*/.exec(line)?.[0] ?? "";
  let spaces = "";
  for (const character of indent) {
    spaces += character === "\t" ? "  " : " ";
  }
  return `${spaces}${line.slice(indent.length)}`;
}

function hygiene(code: string, structural: boolean): string {
  const lines = trimTrailingBlanks(linesOf(code));
  if (!structural) {
    return lines.join("\n");
  }
  while (lines.length > 0 && lines[0] === "") {
    lines.shift();
  }
  const collapsed: string[] = [];
  for (const line of lines) {
    if (line === "" && collapsed.at(-1) === "") {
      continue;
    }
    collapsed.push(line);
  }
  const expanded = collapsed.map(expandLeadingTabs);
  const indents = expanded
    .filter((line) => line !== "")
    .map((line) => /^( *)/.exec(line)?.[1]?.length ?? 0);
  const indent = indents.length > 0 ? Math.min(...indents) : 0;
  return expanded.map((line) => (line === "" ? "" : line.slice(indent))).join("\n");
}

function openWorkspace(): Workspace {
  const {
    Biome: BiomeClass,
  }: typeof import("@biomejs/js-api/nodejs") = require("@biomejs/js-api/nodejs");
  const biome = new BiomeClass();
  const { projectKey } = biome.openProject();
  biome.applyConfiguration(projectKey, configuration);
  return { biome, projectKey };
}

function biomeFormat(code: string, fileName: string): string | undefined {
  workspace ??= openWorkspace();
  let formatted: string | undefined;
  try {
    const { content, diagnostics } = workspace.biome.formatContent(workspace.projectKey, code, {
      filePath: fileName,
    });
    if (!diagnostics.some((diagnostic) => ["error", "fatal"].includes(diagnostic.severity))) {
      formatted = content;
    }
  } catch {}
  return formatted;
}

export function formatFence(language: string | undefined, code: string): string {
  const id = (language ?? "").trim().toLowerCase();
  const text = hygiene(code, !literal.has(id));
  const fileName = biomeFiles.get(id);
  if (!fileName) {
    return text;
  }
  const formatted = biomeFormat(text, fileName);
  return formatted === undefined ? text : hygiene(formatted, false);
}
