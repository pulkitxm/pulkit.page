...
import { query, queryAll } from "../lib/dom.ts";
...
const cards: readonly BounceCard[] = [
  {
    label: "No Bounce",
...
function card({ label, description, bounce }: BounceCard): string {
...
export const mount: DemoMount = (root) => {
  ...
  const boxes = queryAll(root, "[data-box]", HTMLDivElement);
    ...
    boxes.forEach((box, index) => {
      ...
      if (config) {
        animate(
          box,
          { y: isActive ? -60 : 0 },
          { damping: config.damping, mass: 1, stiffness: config.stiffness, type: "spring" },
        );
      }
    });
  }

  query(root, "[data-animate]", HTMLButtonElement).addEventListener("click", () => {
    setActive(true);
    scheduler.later(() => setActive(false), 800);
  });
