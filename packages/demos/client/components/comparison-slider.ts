...
export const mount: DemoMount = (root) => {
  ...
  const top = ref(root, "top", HTMLDivElement);
  const line = ref(root, "line", HTMLDivElement);
  const handle = ref(root, "handle", HTMLDivElement);
  const inset = ref(root, "inset", HTMLSpanElement);
  ...
  function render() {
    ...
    inset.textContent = String(rightInset);
  }
