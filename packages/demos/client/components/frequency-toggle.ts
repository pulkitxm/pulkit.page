...
export const mount: DemoMount = (root) => {
  ...
  let exiting = false;
  let panelAnimation: AnimationPlaybackControlsWithThen | undefined;
  ...
  const placeholder = ref(root, "placeholder", HTMLDivElement);
  const question = ref(root, "question", HTMLParagraphElement);
  ...
  function toggle(animated: boolean) {
  ...
  scheduler.add(() => panelAnimation?.stop());
