import { type AnimationPlaybackControls, animate, type Transition } from "motion";
...
const BALL_SIZE = 32;
const springTransition: Transition = { damping: 20, stiffness: 200, type: "spring" };
...
function position(box: HTMLElement, event: MouseEvent): Point {
...
export const mount: DemoMount = (root) => {
  root.innerHTML = stage(
    ...
    ${caption("CSS transitions restart from scratch when interrupted. Spring animations preserve velocity and curve naturally to the new target.")}
  `,
  );
  const cssBox = ref(root, "cssBox", HTMLButtonElement);
  const cssBall = ref(root, "cssBall", HTMLDivElement);
  const springBox = ref(root, "springBox", HTMLButtonElement);
  const springBall = ref(root, "springBall", HTMLDivElement);
  const springControls: AnimationPlaybackControls[] = [];
