...
import { blockMainThread } from "../lib/main-thread.ts";
...
function boxes(className: string): string[] {
  return Array.from(
    { length: GRID_SIZE },
    (_, i) =>
...
export const mount: DemoMount = (root) => {
  ...
  root.innerHTML = stage(
    ...
    <style>${styles}</style>
    ${caption("When you block the main thread, the left animation freezes because width/height changes require JavaScript. The right animation keeps running because transform is handled entirely by the GPU.", { width: "max-w-sm" })}
  `,
  );
  const block = ref(root, "block", HTMLButtonElement);
  const toggle = ref(root, "toggle", HTMLButtonElement);
  const boxElements = queryAll(root, "[data-box]", HTMLDivElement);
