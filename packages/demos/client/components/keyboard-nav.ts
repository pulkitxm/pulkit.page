...
import { closestTarget, listenWindow, ref } from "../lib/dom.ts";
...
const items: readonly BrowserItem[] = [
...
function highlightTop(index: number): string {
  return `${4 + index * 32}px`;
}
...
export const mount: DemoMount = (root) => {
  ...
  let animated = true;
  let isFocused = false;
  let layoutAnimation: AnimationPlaybackControls | undefined;
  ...
  const browser = ref(root, "browser", HTMLDivElement);
  const hint = ref(root, "hint", HTMLSpanElement);
  const list = ref(root, "list", HTMLDivElement);
  const mode = ref(root, "mode", HTMLSpanElement);
  let highlight = ref(root, "highlight", HTMLDivElement);
  ...
  function select(index: number) {
    ...
    selectedIndex = index;
    if (!animated) {
      highlight.style.top = highlightTop(selectedIndex);
      return;
    }
  ...
  function setAnimated(value: boolean) {
    ...
    replacement.style.top = highlightTop(selectedIndex);
  ...
  list.addEventListener("click", (event) => {
    const target = closestTarget(event, "[data-index]", HTMLElement);
    if (target) {
      select(Number(target.dataset.index));
    }
  });
  scheduler.add(
    listenWindow("keydown", (event) => {
      if (!isFocused) {
        return;
      }
  ...
  scheduler.add(() => layoutAnimation?.stop());
