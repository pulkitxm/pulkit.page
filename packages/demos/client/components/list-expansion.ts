...
import { button, cn, html, type IconNode, icon } from "../runtime/ui.ts";
...
function detailView(item: ListItem): string {
...
function header(title: string, toggleLabel: string, toggleRef: string): string {
...
function content(item: ListItem | null): HTMLDivElement {
...
export const mount: DemoMount = (root) => {
  let selected: ListItem | null = null;
  ...
  const plain = ref(root, "plain", HTMLDivElement);
  const plainBody = ref(root, "plainBody", HTMLDivElement);
  const animated = ref(root, "animated", HTMLDivElement);
  const toAnimated = ref(root, "toAnimated", HTMLButtonElement);
  const toPlain = ref(root, "toPlain", HTMLButtonElement);
  ...
  function setUseAnimation(useAnimation: boolean) {
  ...
  root.addEventListener("click", (event) => {
    const itemId = closestTarget(event, "[data-item]", HTMLElement)?.dataset.item;
    if (itemId) {
      select(items.find((item) => String(item.id) === itemId) ?? null);
    } else if (closestTarget(event, "[data-back]", Element)) {
      select(null);
    }
  });
  ...
  renderPlain();
  const initial = content(null);
  current = initial;
  animated.append(initial);
  animate(initial, { opacity: 0 }, { duration: 0 });
  animate(initial, { opacity: 1 }, fade);
};
