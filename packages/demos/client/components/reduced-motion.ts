...
export const mount: DemoMount = (root) => {
  ...
  const toggle = ref(root, "toggle", HTMLInputElement);
  const trigger = ref(root, "trigger", HTMLButtonElement);
  const menu = ref(root, "menu", HTMLDivElement);
  const note = ref(root, "note", HTMLParagraphElement);
