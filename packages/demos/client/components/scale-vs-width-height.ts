...
import { button, html, icon } from "../runtime/ui.ts";
...
export const mount: DemoMount = (root) => {
  ...
  const sizedButton = expectElement(
    ref(root, "sized", HTMLDivElement).firstElementChild,
    HTMLButtonElement,
  );
  const scaleButton = expectElement(
    ref(root, "scaledBox", HTMLDivElement).firstElementChild,
    HTMLButtonElement,
  );

  function render(scaled: boolean) {
  ...
  ref(root, "slot", HTMLDivElement).replaceWith(swap.element);
  render(false);
};
