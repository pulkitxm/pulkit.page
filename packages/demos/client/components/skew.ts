...
export const mount: DemoMount = (root) => {
  ...
  const xValue = ref(root, "xValue", HTMLSpanElement);
  const yValue = ref(root, "yValue", HTMLSpanElement);
  ...
  ref(root, "xGroup", HTMLDivElement).append(xSlider.element);
  ref(root, "yGroup", HTMLDivElement).append(ySlider.element);
  ref(root, "reset", HTMLButtonElement).addEventListener("click", () => {
    skewX = 0;
    skewY = 0;
    render();
  });
