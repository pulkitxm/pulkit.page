import { animate, type MotionValue, motionValue, type Transition } from "motion";
...
const transition: Transition = { damping: 25, stiffness: 400, type: "spring" };
const toastClass =
...
export const mount: DemoMount = (root) => {
  ...
  const add = ref(root, "add", HTMLButtonElement);
  ...
  function snapshot(): Map<Toast, LayoutBox> {
    return new Map(
      toasts.map((toast) => {
  ...
  function applyLayout(before: Map<Toast, LayoutBox>) {
  ...
  add.addEventListener("click", () => {
    ...
    const toast: Toast = { element, layoutX: motionValue(0), layoutY: motionValue(0) };
    ...
    query(element, "button", HTMLButtonElement).addEventListener("click", () => remove(toast));
    list.append(element);
    toasts.push(toast);
    applyLayout(before);
    animate(element, { opacity: 0, scale: 0.9, y: 50 }, { duration: 0 });
    animate(element, { opacity: 1, scale: 1, y: 0 }, transition);
    scheduler.later(() => remove(toast), 3000);
  });
