import { animate, hover, type Transition } from "motion";
import { ref } from "../lib/dom.ts";
import { button, html } from "../runtime/ui.ts";
import type { DemoMount } from "../types.ts";

const springTransition: Transition = { damping: 17, stiffness: 400, type: "spring" };
...
export const mount: DemoMount = (root) => {
  ...
  const linear = ref(root, "linear", HTMLButtonElement);
  const spring = ref(root, "spring", HTMLButtonElement);

  linear.addEventListener("mouseenter", () => {
    linear.style.transform = "scale(1.05)";
  });
  linear.addEventListener("mouseleave", () => {
    linear.style.transform = "scale(1)";
  });
