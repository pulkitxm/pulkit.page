...
function trackSize(view: DataView, offset: number, end: number): Size | undefined {
...
function findTrackSize(view: DataView, start: number, end: number): Size | undefined {
  let offset = start;
  let found: Size | undefined;
  while (!found && offset + 8 <= end) {
    ...
    if (containers.has(type)) {
      found = findTrackSize(view, offset + 8, offset + size);
    }
    if (!found && type === "tkhd") {
      found = trackSize(view, offset, offset + size);
    }
    offset += size;
  }
  return found;
...
export function localVideoSize(src: string): Size | undefined {
