...
import { optionalString, readString } from "../lib/props.ts";
import type { EmbedRenderer } from "../types.ts";

const contentClasses: Readonly<Record<string, string>> = {
  "my-8": "my-8",
  "h-[200px]": "h-[200px]",
  "w-full": "w-full",
...
export const render: EmbedRenderer = (props, { assets, escapeHtml }) => {
  const videoId = readString(props, "videoId", "youtube-embed");
  const imgLink = optionalString(props, "imgLink", "youtube-embed");
  ...
  const classes = [
    ...new Set(
      `relative aspect-[16/9] w-full overflow-hidden rounded-[12px] border-2 border-[#fff]/30 ${mapClasses(optionalString(props, "className", "youtube-embed"))}`.split(
        " ",
      ),
    ),
  ]
