...
import { renderMarkdown } from "./markdown-export.ts";
...
function inlineEmbed(name: string, props: Record<string, string>): string {
  return `:embed[${name}]{${JSON.stringify(props)}}`;
}

function page(body: string, route = "/blogs/first/"): PageRecord {
  const entry = pages.find((candidate) => candidate.route === route);
  if (!entry) {
    throw new Error(`Unknown test route ${route}`);
  }
  return { ...entry, body };
}
...
test("articles at the site root get their own writing section", () => {
  const blog: Site = { ...site, articles: "/" };
  const rootPages: ListedPage[] = [
    ...pages.slice(0, 1),
    {
      route: "/post/",
      index: false,
