...
import { readFrontmatter, readPage, siteSettings } from "../site/read-page.ts";
...
const site: Site = {
  ...siteSettings(readFrontmatter(readFileSync("content/_site.md", "utf8")).data),
  articles: "/blogs/",
...
test("graphs distinguish collections, posts, contact and experience without invented dates", () => {
  ...
  expect(article?.author).toEqual({ "@id": `${site.url}/#person` });
});
...
test("validator rejects wrong canonicals, missing metadata, broken JSON and wrong PNG dimensions", async () => {
  ...
  const validate = (value: string, buffer = png) =>
...
test("sitemap dates articles by publication and leaves other pages undated", () => {
  ...
  const sitemap = String(crawlerOutputs(dated, site).get("sitemap.xml"));
