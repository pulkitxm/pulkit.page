...
import { assetFile, themeFile } from "./files.mjs";

function withAppAssets(files, check) {
...
test("theme files resolve inside the theme package", () => {
  expect(themeFile("theme.js")).toMatch(/packages\/theme\/theme\.js$/);
...
test("paths outside /assets/, missing files and traversal resolve to nothing", () => {
  withAppAssets({ "a.txt": "a" }, (assets) => {
    ...
    expect(assetFile("/assets/../../theme.js", assets)).toBeUndefined();
  });
});
