...
export async function scanText(file, source) {
  ...
  let ranges;
  if (file.startsWith("content/blogs/") && extension === "md") {
    ranges = await markdownRanges(source, true);
  } else if (file.startsWith("pages/blogs/") && extension === "html") {
    ranges = await htmlRanges(source, true);
  } else {
    ranges = await commentRanges(language, source);
  }
