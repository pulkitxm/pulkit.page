...
test("Vite serves pages on demand, refreshes dependencies, and recovers after errors", async () => {
  ...
  for (const path of ["scripts", "layouts", "assets/fonts"]) {
  ...
  writeFileSync(join(directory, "theme.js"), "document.documentElement.dataset.test = 'ok';\n");
  const child = spawn("bun", [join(directory, "scripts/dev.mjs")], {
    cwd: directory,
  ...
  try {
    ...
    expect((await fetch(`${origin}node_modules/vite/dist/client/env.mjs`)).status).toBe(200);
    ...
    expect(output).not.toContain("GET /node_modules/vite/dist/client/env.mjs");
...
test("Vite rejects invalid and occupied explicit ports", () => {
  for (const port of ["invalid", "-1", "65536", "3000.5"]) {
    const result = spawnSync("bun", [join(root, "scripts/dev.mjs")], {
      cwd: root,
