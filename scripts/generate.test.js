...
function project() {
  ...
  cpSync(join(root, "demos"), join(directory, "demos"), { recursive: true });
  cpSync(join(root, "client"), join(directory, "client"), { recursive: true });
  cpSync(join(root, "bun.lock"), join(directory, "bun.lock"));
...
function build(cwd, env = fixtureEnv) {
  return spawnSync("bun", [join(cwd, "scripts/build.mjs")], { env, cwd, encoding: "utf8" });
}
...
test("pre-commit checks the staged snapshot, not unstaged repairs", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ type: "module", scripts: { ci: "bun scripts/check-content.mjs" } }),
  );
  cpSync(join(root, "content/_site.md"), join(cwd, "content/_site.md"));
  const broken = source.replace("description: Example page.\n", "");
  ...
  expect(readFileSync(join(cwd, "content/home.md"), "utf8")).toBe(source);
...
test("rebuilds reuse cached renders and invalidate on content, layout and config edits", () => {
  const cwd = project();
