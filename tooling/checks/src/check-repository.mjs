...
for (const file of files) {
  ...
  if (
    /^(?:apps\/[a-z0-9-]+\/(?:content|layouts)|packages\/[a-z0-9-]+\/src|tooling\/[a-z0-9-]+\/src|docs)\//.test(
      file,
    ) &&
    !file.endsWith("/content/_site.md") &&
  ...
  const limit =
    /^apps\/[a-z0-9-]+\/assets\/content\//.test(file) && /\.(gif|webp|png|jpg|pdf)$/.test(file)
      ? 5 * 1024 * 1024
      : maximumBytes;
  if (size > limit) {
