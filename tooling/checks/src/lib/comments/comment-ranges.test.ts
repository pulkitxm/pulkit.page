...
import { commentRanges, scanText } from "./comment-ranges.ts";
...
describe("comment detection", () => {
  ...
  ])("classifies comments in %s", async (file: string, source: string, count: number) => {
  ...
  test("article examples retain comments while source comments remain forbidden", async () => {
    ...
    expect(await scanText("apps/page/content/blogs/example.md", article)).toHaveLength(2);
